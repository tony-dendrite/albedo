"""albedo.models.ref — immutable model reference (repo + content digest)."""
from __future__ import annotations

import re
from dataclasses import dataclass

# Lowercase Hippius "<namespace>/<name>" id. The strict challenger naming rule
# (REPO_PATTERN) is enforced at admission/upload time, not on every ref, so a
# king/genesis repo need not follow the challenger naming convention.
...
class ModelRef:
    ...
    def __repr__(self) -> str:  # pragma: no cover
        return f"ModelRef(repo={self.repo!r}, digest={self.digest!r})"
