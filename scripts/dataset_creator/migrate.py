...
def migrate_if_needed(cfg: Config, state: State) -> None:
    ...
    if legacy_json.exists():
        ...
        for run_id, meta in data.get("processed_runs", {}).items():
            state.mark_processed(run_id, meta.get("rows", 0), meta.get("source", "db"),
                                 meta.get("ingested_at"), meta.get("started_at"))
    ...
    if all_dir.is_dir():
        ...
        for p in sorted(all_dir.glob("*.parquet")):
            ...
            if len(rem):
                rows = [{"sample_id": rec["sample_id"],
                         "messages": [dict(m) for m in rec["messages"]],
                         "_run_id": rec["_run_id"]} for rec in rem.to_dict("records")]
