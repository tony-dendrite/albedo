...
def preflight() -> int:
    """Number of banked models, raising if the index is missing or holds none.

    Every other call here routes through ensure_index(), which CREATES the index when it is
    absent.  That is right for a seeded deployment and wrong at startup: a typo in
    OPENSEARCH_URL or OPENSEARCH_INDEX would silently make an empty bank, and the operator
    would see only a stream of retryable "dedup bank is empty" infra faults (gate.run) with
    nothing naming the cause.  Checked once at worker start, against the index as it is.
    """
    name = index_name()
    c = get_client()
    if not c.indices.exists(index=name):
        raise RuntimeError(
            f"dedup index {name!r} does not exist at {config.OPENSEARCH_URL} — check "
            "ALBEDO_OPENSEARCH_URL/ALBEDO_OPENSEARCH_INDEX, then seed it with "
            "scripts/dedup_seed_bank.py"
        )
    n = int(c.count(index=name, body={"query": {"term": {"status": STATUS_BANK}}})["count"])
    if not n:
        raise RuntimeError(
            f"dedup index {name!r} holds no status={STATUS_BANK} documents — bootstrap it with "
            "scripts/dedup_seed_bank.py before starting the validator"
        )
    return n
