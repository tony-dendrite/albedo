...
def test_challenger_win_requires_six_percent_margin():
    ...
    assert challenger_beats_king(0.37, 0.30) is True   # 0.07 >= 0.06
    assert challenger_beats_king(0.34, 0.30) is False  # 0.04 < 0.06
...
def test_aggregate_scores_crowns_on_margin():
    ...
    below = aggregate_scores([_record(0.30, 0.32) for _ in range(10)])  # Δ 0.02 < 0.06 margin
