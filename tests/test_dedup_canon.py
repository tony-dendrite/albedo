...
def test_an_empty_model_dir_names_itself_instead_of_raising_keyerror(tmp_path):
    """cleanup_models.sh can delete a snapshot mid-run, including the canon seed's.

    Provider used to accept the empty directory and fail later, deep in fingerprint(), as
    KeyError('embed_tokens.weight') - naming neither the path nor which of the two models had
    gone.  Both sides go through Provider, so the message has to carry the directory.
    """
    gone = tmp_path / "gone"
    gone.mkdir()
    with pytest.raises(RuntimeError) as exc:
        canon.Provider(str(gone), CPU)
